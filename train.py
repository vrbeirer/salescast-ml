...
split_res = items_df.item_name.str.split("[", n=1, expand=True)
items_df["name1"] = split_res[0]
items_df["name2"] = split_res[1] if split_res.shape[1] > 1 else None
split_res2 = items_df.item_name.str.split("(", n=1, expand=True)
items_df["name1"] = split_res2[0]
items_df["name3"] = split_res2[1] if split_res2.shape[1] > 1 else None
...
matrix = pd.concat([matrix, test_df.drop(["ID"],axis = 1)], ignore_index=True, sort=False)
